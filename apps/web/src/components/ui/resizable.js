...
const ResizablePanelGroup = ({ className, direction, orientation = direction ?? "horizontal", ...props }) => (_jsx(PanelGroupDirectionContext.Provider, { value: orientation, children: _jsx(Group, { className: cn("flex h-full w-full data-[panel-group-direction=vertical]:flex-col", className), "data-panel-group-direction": orientation, orientation: orientation, ...props }) }));
